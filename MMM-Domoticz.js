  ...
  render: function(data) {
    ...
    if (this.config.subMenus === true) {
      var therm =
        ...
        this.config.temperatureTitle +
        "</header><table" +
        this.setTextColour() +
        ">";
      var power =
        '<header class="module-header">' +
        this.config.energyTitle +
        "</header><table" +
        this.setTextColour() +
        ">";
      var batt =
        '<header class="module-header">' +
        this.config.batteryTitle +
        "</header><table" +
        this.setTextColour() +
        ">";
      var co =
      ...
      if (this.config.excludeDevices.indexOf(dev.Name) == -1) {
        ...
        if (dev.SwitchType == "Blinds" || dev.SwitchType == "Blinds Inverted") {
          ...
          blinds +=
            ...
            dev.Name +
            '</td><td class="small ' +
            (dev.Status == "Closed" ? "yellow" : "") +
            '"><i class="fa fa-arrow-' +
            (dev.Status == "Closed" ? "down" : "up") +
            '"></i></td></tr>';
        }
        if (dev.BatteryLevel <= this.config.batteryThreshold) {
          ...
          batt +=
            '<tr><td class="small ' +
            (dev.BatteryLevel < this.config.batteryThreshold - 8 ? "red" : "") +
            '">' +
            dev.Name +
            '</td><td class="small ' +
            (dev.BatteryLevel < 15 ? "red" : "") +
        ...
        if (dev.Name == "Domoticz Security Panel") {
          ...
          alarm +=
            ...
            dev.Name +
            '</td><td class="small ' +
            (dev.Data == "Normal" ? this.config.alarmOffLabel : dev.Data) +
            '"></td></tr>';
        }
        ...
        if (
          dev.Type.indexOf("Humidity") > -1 &&
          this.config.showItems.indexOf("humidity") !== -1
        ) {
          ...
          if (this.config.groupSensors === true) {
            ...
            therm +=
              '<tr><td class="small">' +
              (tempName != dev.Name ? dev.Name : hookdir) +
              '</td><td class="small">';
            therm +=
              parseInt(dev.Humidity) + '% <i class="fa fa-tint"></i></td></tr>';
            // set a temporary name to prevent device names end double in groups
            tempName = dev.Name;
          } else {
            if (this.config.showItems.indexOf("humidity")) {
              humi +=
                '<tr><td class="small">' + dev.Name + '</td><td class="small">';
              humi +=
                parseInt(dev.Humidity) +
                '% <i class="fa fa-tint"></i></td></tr>';
            }
          }
        }
        if (
          dev.Type.indexOf("Baro") > -1 &&
          this.config.showItems.indexOf("baro") !== -1
        ) {
    ...
    if (alarmCount > 0) {
      text += this.config.showItems.indexOf("alarm") !== -1 ? alarm : "";
    }
    if (voltageCount > 0) {
      text += this.config.showItems.indexOf("voltage") !== -1 ? voltage : "";
    }
    if (batteryCount > 0) {
    ...
    if (this.config.showItems.indexOf("usage") !== -1) {
      ...
      text +=
        '<tr><td class="small">' +
        this.config.energyNow +
        '</td><td class="small">' +
        parseFloat(powerUse).toFixed(1) +
        " Watt</td></tr>";
      text +=
        '<tr><td class="small">' +
        this.config.energyToday +
        '</td><td class="small">' +
